...
async function runBrowserEarningAndUserMgmtTests() {
    ...
    try {
        ...
        // سشن کاربر عادی باید کنار گذاشته شود، وگرنه /login به داشبورد
        // ریدایرکت می‌شود و فرم ورودی وجود ندارد (منشأ TimeoutError پیشین).
        // مسیر /logout فقط POST + CSRF است، پس ساده‌ترین راه تمیز، پاک کردن
        // کوکی‌های همین context است.
        await context.clearCookies();
        await page.goto(`${BASE_URL}/login`, { waitUntil: 'networkidle' });
        // این بخش پنل حاکمیتی را می‌سنجد، پس باید با حساب ادمین وارد شد نه کاربر عادی.
        await page.fill('input[name="identifier"], input[name="email"]', E2E_ADMIN_EMAIL);
