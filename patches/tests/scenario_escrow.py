...
def test_escrow_L2_release_escrow_success(client, assertions):
    ...
    _c, _b, _ = client.post('/wallet/escrow/store', {
        'seller': 'e.L2.2_s@chortke.test',
        'amount': '300000',
        'title': 'اسکرو آزادسازی'
    }, csrf_token=token, page_body=body)
    assert_true(assertions, f"ایجاد اسکرو HTTP {_c}", _c == 200 and '"success": true' in _b)

    escrow_id = db_scalar(f"SELECT id FROM escrow_transactions WHERE buyer_id={uid_buyer} ORDER BY id DESC LIMIT 1")
    assert_true(assertions, f"اسکرو در DB ایجاد شد (id: {escrow_id})", bool(escrow_id))
    ...
    assert_true(assertions, f"آزادسازی اسکرو HTTP {code}", code == 200 and '"success": true' in body)
...
def test_escrow_L6_escrow_release_race_condition(client, assertions):
    ...
    # جدول escrows ستون‌های title/updated_at ندارد؛ INSERT پیشین بی‌صدا شکست
    # می‌خورد، escrow_id خالی می‌ماند و درخواست‌های «همزمان» روی اسکروی
    # ناموجود اجرا می‌شدند — یعنی ادعای جلوگیری از واریز دوباره چیزی را
    # اثبات نمی‌کرد. ستون شناسه‌دار transaction_id برای یافتن دقیق رکورد به‌کار می‌رود.
    escrow_ref = f"escrow-race-{int(time.time())}-{uid_b}"
    db_insert(
        "INSERT INTO escrows (transaction_id, buyer_id, seller_id, amount, currency, status, created_at) "
        f"VALUES ('{escrow_ref}', {uid_b}, {uid_s}, 300000, 'irt', 'held', NOW())"
    )
    escrow_id = db_scalar(f"SELECT id FROM escrows WHERE transaction_id='{escrow_ref}'")
    assert_true(assertions, f"اسکرو آزمایشی ساخته شد (id={escrow_id})", str(escrow_id).isdigit())
