...
total = len(infra_checks)
failed = [name for name, st, _ in infra_checks if not st]
print(f"نتیجه ممیزی زیرساخت Production: {ready_count} از {total} آیتم پیکربندی و آماده‌سازی شده‌اند.")
if failed:
    print(f"\n{len(failed)} آیتم آماده نیست:")
    for name in failed:
        print(f"  ✗ {name}")
print("=" * 80)

# این ممیزی باید بتواند شکست بخورد. پیش‌تر هیچ sys.exit وجود نداشت و اسکریپت
# حتی با رد شدن هر ۱۲ بررسی، کد خروج ۰ برمی‌گرداند — یعنی در CI همیشه سبز بود.
