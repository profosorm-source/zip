...
def test_maintenance_L1_smoke_maintenance_page(client, assertions):
    ...
    # هیچ مسیری با الگوی maintenance در routes/*.php ثبت نشده است.
    # پذیرش ۴۰۴ صرفاً نبودِ این قابلیت را پنهان می‌کرد.
    skip_scenario(assertions, "مسیر /admin/maintenance در محصول پیاده‌سازی نشده است")
...
def test_maintenance_L1_smoke_system_setting_page(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body = client.get('/admin/settings')
    assert_true(assertions, f"صفحه تنظیمات کلان HTTP {code}", code in (200, 302))
...
def test_maintenance_L7_browser_maintenance_toggle_nav(client, assertions):
    ...
    # همانند L1: مسیر /admin/maintenance در محصول ثبت نشده است.
    skip_scenario(assertions, "مسیر /admin/maintenance در محصول پیاده‌سازی نشده است")
