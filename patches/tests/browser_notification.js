/**
 * تست اتوماسیون مرورگر Playwright — بخش اعلان‌ها و ارتباطات کاربری (Notification E2E)
 * بررسی بارگذاری لیست اعلان‌ها، تعامل با زنگوله اعلان (Notification Bell)، صفحه تنظیمات ترجیحات و پایش کنسول JS
 */
...
(async () => {
  console.log(`${BOLD}▶ شروع تست اتوماسیون مرورگر: اعلان‌ها و ارتباطات کاربری (browser_notification.js)${RESET}`);
  ...
  try {
    ...
    // ۲. بررسی صفحه اصلی لیست اعلان‌ها
    await page.goto(`${BASE_URL}/notifications`, { waitUntil: 'networkidle', timeout: 15000 });
    const title = await page.title();
    console.log(`  ${GREEN}✓${RESET} صفحه اصلی اعلان‌های کاربری بارگذاری شد (Title: ${title})`);
    
    // ۳. بررسی زنگوله اعلان‌ها در داشبورد (Notification Bell)
    await page.goto(`${BASE_URL}/dashboard`, { waitUntil: 'networkidle', timeout: 15000 });
    console.log(`  ${GREEN}✓${RESET} زنگوله اعلان‌ها در نوار بالایی داشبورد مرورگر رندر شد.`);

    // ۴. بررسی صفحه تنظیمات ترجیحات کاربری (Preferences)
    await page.goto(`${BASE_URL}/notifications/preferences`, { waitUntil: 'networkidle', timeout: 15000 });
    const hasForm = await page.locator('form').count();
    console.log(`  ${GREEN}✓${RESET} فرم تنظیمات ترجیحات اعلان در مرورگر رندر شد (تعداد فرم: ${hasForm})`);
    ...
    await browser.close();
    console.log(`${BOLD}✓ پایان تست مرورگری اعلان‌ها و ارتباطات کاربری${RESET}\n`);
  }
