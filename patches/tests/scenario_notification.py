...
def test_notification_L1_smoke_unread_count(client, assertions):
    ...
    assert_true(assertions, f"اندپوینت شمارش اعلان HTTP {code}", code in (200, 302))
...
def test_notification_L1_smoke_preferences_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه تنظیمات اعلان HTTP {code}", code in (200, 302))
