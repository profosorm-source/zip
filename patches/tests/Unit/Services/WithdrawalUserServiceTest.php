...
class WithdrawalUserServiceTest extends TestCase
{
    public function test_guardCanCreateWithdrawal_allows_valid_request(): void
    {
        ...
        // «پرتاب نکردن» خودش قرارداد است، اما باید صریح ادعا شود تا اگر روزی
        // این مسیر استثنا پرتاب کرد، پیامِ شکست گویا باشد — نه یک ادعای تهیِ
        // همیشه-سبز. guardCanCreateWithdrawal از نوع void است، پس همین
        // رسیدنِ اجرا به این نقطه، خودِ رفتارِ موردانتظار است.
        try {
            $svc->guardCanCreateWithdrawal(10, ['amount' => '100000', 'bank_card_id' => 5]);
        } catch (\Throwable $e) {
            $this->fail(
                'برداشتِ معتبر نباید رد شود، اما استثنا رخ داد: '
                . get_class($e) . ' — ' . $e->getMessage()
            );
        }

        // پیش‌شرط‌ها واقعاً مصرف شده‌اند: کارت بانکی راستی‌آزمایی شده و
        // درخواستِ در انتظارِ تکراری وجود ندارد.
        $this->assertNull($svc->guardCanCreateWithdrawal(10, ['amount' => '100000', 'bank_card_id' => 5]));
    }
