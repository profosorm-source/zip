...
class TicketServiceTest extends TestCase
{
    ...
    public function guard_can_create_ticket_validates_successfully(): void
    {
        ...
        // ادعای واقعی: به‌جای «استثنا نداد»، بررسی می‌کنیم دقیقاً چه چیزی به
        // لایهٔ اعتبارسنجی می‌رسد — همان جایی که TicketCommandService
        // پیش‌فرض‌ها را ادغام و قواعد را تعریف می‌کند.
        $seenData = null;
        $seenRules = null;
        $this->validatorFactory->shouldReceive('make')
            ->once()
            ->andReturnUsing(function (array $d, array $r = []) use ($validator, &$seenData, &$seenRules) {
                $seenData = $d;
                $seenRules = $r;
                return $validator;
            });
        ...
        $this->assertIsArray($seenData, 'اعتبارسنج هرگز فراخوانی نشد.');
        $this->assertSame($data['subject'], $seenData['subject']);
        $this->assertSame($data['message'], $seenData['message']);
        $this->assertSame('billing', $seenData['category']);
        $this->assertSame('normal', $seenData['priority']);
        $this->assertSame('some_unique_key_123456789', $seenData['idempotency_key']);

        $this->assertIsArray($seenRules);
        foreach (['subject', 'message', 'category', 'priority', 'idempotency_key'] as $field) {
            $this->assertArrayHasKey($field, $seenRules, "قاعدهٔ اعتبارسنجی برای {$field} تعریف نشده است.");
            $this->assertStringContainsString('required', (string)$seenRules[$field]);
        }
    }
