...
class UserControllersStructuralTest extends TestCase
{
    ...
    public function testPublicMethodsReturnVoidOrResponse(string $class): void
    {
        ...
        // به‌جای ادعای تهی: خودِ کلاس باید متدِ عمومیِ قابل‌بازرسی داشته باشد،
        // وگرنه حلقهٔ بالا هرگز چیزی نسنجیده و تست بی‌معنا سبز می‌شود.
        $ownPublic = array_filter(
            $methods,
            static fn(\ReflectionMethod $m): bool =>
                $m->getDeclaringClass()->getName() === $class && $m->getName() !== '__construct'
        );
        $this->assertNotEmpty(
            $ownPublic,
            "$class هیچ متد عمومیِ اختصاصی ندارد؛ این تست چیزی را راستی‌آزمایی نکرده است."
        );
    }
