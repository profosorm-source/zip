...
class StabilityTest extends TestCase
{
    ...
    public function database_reset_only_allowed_in_cli(): void
    {
        ...
        // رفتار واقعی سنجیده می‌شود نه صرفِ «پرتاب نکردن»:
        // reset() باید singleton را واقعاً پاک کند.
        $ref = new \ReflectionClass(\Core\Database::class);
        $instanceProp = $ref->getProperty('instance');
        $instanceProp->setAccessible(true);

        $sentinel = $ref->newInstanceWithoutConstructor();
        $instanceProp->setValue(null, $sentinel);
        $this->assertSame($sentinel, $instanceProp->getValue(), 'پیش‌شرط برقرار نشد.');

        \Core\Database::reset();

        $this->assertNull($instanceProp->getValue(), 'reset() باید singleton را پاک کند.');
    }
    ...
    public function safe_expression_parser_blocks_dangerous_functions(): void
    {
        ...
        foreach ($dangerous as $expr) {
            try {
                ...
                // ادعای واقعی: استثنا باید پیامی معنادار داشته باشد، وگرنه
                // صِرفِ «پرتاب شدن» چیزی دربارهٔ درستیِ نگهبان نمی‌گوید.
                $this->assertNotSame('', trim($e->getMessage()),
                    "SafeExpression باید دلیلِ ردِ '{$expr}' را گزارش کند.");
            }
        }
    }
