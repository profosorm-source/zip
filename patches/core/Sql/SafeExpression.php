...
final class SafeExpression
{
    ...
    private function parsePrimary(): array
    {
        ...
        if ($tok['type'] === 'PLACEHOLDER') { $this->consume(); return ['kind' => 'placeholder']; }
        ...
        if ($tok['type'] === 'IDENT') {
            ...
            // Read the lookahead once; repeated peek() calls after consume()
            // return different tokens and confuse static narrowing.
            $after = $this->peek();
            ...
            if ($after['type'] === 'PUNCT' && $after['value'] === '.') {
                $this->consume(); // dot
                $next = $this->peek();
                $isIdent = $next['type'] === 'IDENT';
                $isStar  = $next['type'] === 'OP' && $next['value'] === '*';
                if (!$isIdent && !$isStar) {
    ...
    {
        switch ($n['kind']) {
            case 'num':         return $this->nodeStr($n, 'value');
            case 'str':         return "'" . str_replace("'", "''", $this->nodeStr($n, 'value')) . "'";
            case 'placeholder': return '?';
            case 'const':       return $this->nodeStr($n, 'value');
            case 'star':        return '*';
            case 'column':
                $table = $n['table'] ?? null;
                return $table !== null
                    ? '`' . $this->nodeStr($n, 'table') . '`.`' . $this->nodeStr($n, 'name') . '`'
                    : '`' . $this->nodeStr($n, 'name') . '`';
            case 'paren':       return '(' . $this->emitNode($this->nodeAst($n, 'expr')) . ')';
            case 'neg':         return '-' . $this->emitNode($this->nodeAst($n, 'expr'));
            case 'not':         return 'NOT ' . $this->emitNode($this->nodeAst($n, 'expr'));
            case 'is_null':     return $this->emitNode($this->nodeAst($n, 'expr')) . ' IS ' . ($n['negate'] ? 'NOT ' : '') . 'NULL';
            case 'cmp':         return $this->emitNode($this->nodeAst($n, 'left')) . ' ' . $this->nodeStr($n, 'op') . ' ' . $this->emitNode($this->nodeAst($n, 'right'));
            case 'arith':       return $this->emitNode($this->nodeAst($n, 'left')) . ' ' . $this->nodeStr($n, 'op') . ' ' . $this->emitNode($this->nodeAst($n, 'right'));
            case 'logical':     return $this->emitNode($this->nodeAst($n, 'left')) . ' ' . $this->nodeStr($n, 'op') . ' ' . $this->emitNode($this->nodeAst($n, 'right'));
            case 'between':     return $this->emitNode($this->nodeAst($n, 'expr')) . ' BETWEEN ' .
                                       $this->emitNode($this->nodeAst($n, 'lo')) . ' AND ' . $this->emitNode($this->nodeAst($n, 'hi'));
            case 'like':        return $this->emitNode($this->nodeAst($n, 'left')) . ' LIKE ' . $this->emitNode($this->nodeAst($n, 'right'));
            case 'in':
                $items = array_map(fn($x) => $this->emitNode($x), $this->nodeList($n, 'items'));
                return $this->emitNode($this->nodeAst($n, 'expr')) . ' IN (' . implode(', ', $items) . ')';
            case 'case':
                $s = 'CASE';
                foreach ($this->nodeList($n, 'branches') as $b) {
                    $s .= ' WHEN ' . $this->emitNode($this->nodeAst($b, 'when')) . ' THEN ' . $this->emitNode($this->nodeAst($b, 'then'));
                }
                if ($n['else'] !== null) $s .= ' ELSE ' . $this->emitNode($this->nodeAst($n, 'else'));
                return $s . ' END';
            case 'func':
                $args = array_map(fn($x) => $this->emitNode($x), $this->nodeList($n, 'args'));
                $prefix = $n['distinct'] ? 'DISTINCT ' : '';
                return strtoupper($this->nodeStr($n, 'name')) . '(' . $prefix . implode(', ', $args) . ')';
            case 'interval':
                return 'INTERVAL ' . $this->emitNode($this->nodeAst($n, 'value')) . ' ' . $this->nodeStr($n, 'unit');
            case 'aliased':
                // BUGFIX-SAFEEXPR-AS-ALIAS-2026-06: alias در emit همیشه با AS صریح و backtick.
                return $this->emitNode($this->nodeAst($n, 'expr')) . ' AS `' . $this->nodeStr($n, 'alias') . '`';
        }
        throw new SqlExpressionException("internal: unknown node kind '" . $this->nodeStr($n, 'kind') . "'");
    }

    /**
     * Narrow an AST scalar field to a string. All leaf scalar fields are
     * guaranteed by the parser to be strings at runtime.
     * @param array<string, mixed> $n
     */
    private function nodeStr(array $n, string $key): string
    {
        /** @var string $v */
        $v = $n[$key];
        return $v;
    ...
    /**
     * Narrow a recursive AST node field to an array node.
     * @param array<string, mixed> $n
     * @return array<string, mixed>
     */
    private function nodeAst(array $n, string $key): array
    {
    ...
    /**
     * Narrow a list-of-nodes field.
     * @param array<string, mixed> $n
     * @return list<array<string, mixed>>
     */
    private function nodeList(array $n, string $key): array
    {
    ...
    /**
     * Reads relative to the mutable cursor: the same call can yield different
     * tokens after consume(), so it must not be treated as pure.
     *
     * @phpstan-impure
