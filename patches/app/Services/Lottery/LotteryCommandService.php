        ...
        try {
            return $this->idempotencyService->executeWithTransaction(
                ...
                function() use ($userId, $roundId, $idempotencyKey) {
                    ...
                    // BUG FIX: پنجره‌ی زمانی دوره اصلاً اعمال نمی‌شد. وضعیت 'active' فقط
                    // یک پرچم دستی است و هیچ فرآیندی آن را در پایان دوره نمی‌بندد، بنابراین
                    // کاربر می‌توانست در دوره‌ی منقضی‌شده (یا هنوز شروع‌نشده) بلیت بخرد و
                    // وجه از کیف پولش کسر شود. اعتبارسنجی زمانی زیر همان قفل FOR UPDATE
                    // انجام می‌شود تا با مسیر مشارکت هم‌روند بماند.
                    $nowTs = time();
                    $startDate = str_value($round->start_date ?? '');
                    if ($startDate !== '' && strtotime($startDate) > $nowTs) {
                        throw new \RuntimeException('این دوره از قرعه‌کشی هنوز آغاز نشده است.');
                    }
                    $endDate = str_value($round->end_date ?? '');
                    if ($endDate !== '' && strtotime($endDate) < $nowTs) {
                        throw new \RuntimeException('مهلت شرکت در این دوره از قرعه‌کشی به پایان رسیده است.');
                    }
