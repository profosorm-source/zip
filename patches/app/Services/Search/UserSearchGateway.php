...
class UserSearchGateway extends AbstractSearchGateway
{
    ...
    /** @var array<string, array{table:string, alias:string, columns:list<string>, select:string, module:string}> */
    ...
    private function searchLive(
        ...
        try {
            ...
            $rawItems = $this->db->fetchAll(
                "SELECT {$select} FROM {$table} {$alias} WHERE {$whereSql} ORDER BY {$alias}.created_at DESC LIMIT {$limit} OFFSET {$offset}",
                $params
            );
        } catch (\Throwable $e) {
        ...
        // database outage. Do not turn it into an apparently valid empty result.
