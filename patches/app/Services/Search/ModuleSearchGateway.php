...
final class ModuleSearchGateway extends AbstractSearchGateway
{
    ...
    private function searchVitrineRead(string $q, array $filters, int $limit, int $offset): array
    {
        ...
        try {
            ...
            $rawItems = $this->db->fetchAll("SELECT vl.* FROM vitrine_listings vl WHERE {$whereSql} ORDER BY vl.created_at DESC LIMIT {$limit} OFFSET {$offset}", $params);
        } catch (\Throwable $e) {
            $this->logger->warning('search.vitrine_failed', ['error' => $e->getMessage()]);
            return ['items' => [], 'total' => 0, 'facets' => []];
        }

        // A malformed database result is an internal contract violation; do
        // not hide it behind an empty search response.
    ...
    private function toObjectArray(mixed $rows): array
    {
        if (!is_array($rows) || !array_is_list($rows)) {
            throw new \UnexpectedValueException('Vitrine database result must be an array of rows.');
        }
        ...
        foreach ($rows as $row) {
            if (!$row instanceof \stdClass) {
                throw new \UnexpectedValueException('Vitrine database rows must be stdClass values.');
            }
            $result[] = $row;
        }
        return $result;
