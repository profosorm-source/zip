...
class ReconciliationService
{
    ...
    private function requireTransactionRow(object $transaction): object
    {
        ...
        // فیلدهای اختیاری/nullable: نبودِ ستون در نتیجه‌ی SELECT خطا نیست.
        // مصرف‌کننده‌ها همه‌جا با الگوی `$transaction->transaction_id ?? null`
        // این فیلدها را اختیاری فرض می‌کنند، پس گارد نیز به‌جای رد کردنِ ردیف،
        // آن را به شکل کامل TransactionRow نرمال می‌کند. فقط مقدارِ حاضر ولی
        // غیراسکالر (مثلاً array/object) همچنان نامعتبر است.
        $normalized = clone $transaction;
        foreach (['user_id', 'transaction_id', 'metadata'] as $field) {
            if (!array_key_exists($field, $values)) {
                /** @phpstan-ignore-next-line property.notFound (نرمال‌سازی ردیف ناقص به شکل TransactionRow) */
                $normalized->{$field} = null;
                continue;
            }
            if ($values[$field] !== null && !is_scalar($values[$field])) {
                throw new \UnexpectedValueException("Invalid transaction row: {$field}");
            }
        }

        /** @var TransactionRow $normalized */
        return $normalized;
    }
