...
class IdempotencyCommand
{
    ...
    public function run(array $argv): void
    {
        ...
        if (!is_string($command)) {
            throw new \InvalidArgumentException('Idempotency command name must be a string.');
        }
        ...
        switch ($command) {
            ...
            case 'idempotency:cleanup':
                $this->cleanup($this->parseBooleanOption($opts, 'dry-run'));
                return;
            default:
    ...
    private function parseBooleanOption(array $options, string $name): bool
    {
        ...
        if (!is_string($value)) {
            throw new \InvalidArgumentException("Option --{$name} must be a boolean flag.");
        }
        ...
        throw new \InvalidArgumentException("Option --{$name} must be true or false.");
    }
