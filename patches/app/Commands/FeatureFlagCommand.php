...
class FeatureFlagCommand
{
    ...
    public function run(array $argv): void
    {
        $action = $argv[1] ?? 'help';
        if (!is_string($action)) {
            $this->usageError('Command name must be a string.');
        }
        ...
        switch ($action) {
            ...
            case 'feature:enable':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:enable <name>');
                $this->enable($name);
                break;

            case 'feature:disable':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:disable <name>');
                $this->disable($name);
                break;

            case 'feature:status':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:status <name>');
                $this->status($name);
                break;

            case 'feature:create':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:create <name> <description>');
                $description = $this->requiredStringArgument($argv, 3, 'Usage: php cli.php feature:create <name> <description>');
            ...
            case 'feature:delete':
                ...
                $argsWithoutFlags = array_values(array_filter(
                    $argv,
                    static fn(mixed $value): bool => $value !== '--force'
                ));
                $name = $this->requiredStringArgument(
                    $argsWithoutFlags,
                    2,
                    'Usage: php cli.php feature:delete <name> [--force]'
                );
                $this->delete($name, $force);
                break;

            case 'feature:rollout':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:rollout <name> <percentage>');
                $percentage = $this->requiredIntegerArgument($argv, 3, 'Usage: php cli.php feature:rollout <name> <percentage>');
                $this->rollout($name, $percentage);
                break;

            case 'feature:schedule':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:schedule <name> <from> <until>');
                $from = $this->requiredStringArgument($argv, 3, 'Usage: php cli.php feature:schedule <name> <from> <until>');
                $until = $this->requiredStringArgument($argv, 4, 'Usage: php cli.php feature:schedule <name> <from> <until>');
                $this->schedule($name, $from, $until);
                break;

            case 'feature:history':
                $name = $this->requiredStringArgument($argv, 2, 'Usage: php cli.php feature:history <name> [limit]');
                $limit = array_key_exists(3, $argv)
                    ? $this->requiredIntegerArgument($argv, 3, 'Usage: php cli.php feature:history <name> [limit]')
                    : 20;
                $this->history($name, $limit);
                break;
            ...
            case 'feature:cleanup-metrics':
                $days = array_key_exists(2, $argv)
                    ? $this->requiredIntegerArgument($argv, 2, 'Usage: php cli.php feature:cleanup-metrics [days]')
                    : 30;
                $this->cleanupMetrics($days);
                break;
    ...
    /** @param list<mixed> $argv */
    private function requiredStringArgument(array $argv, int $index, string $usage): string
    {
    ...
    /** @param list<mixed> $argv */
    private function requiredIntegerArgument(array $argv, int $index, string $usage): int
    {
        $value = $argv[$index] ?? null;
