...
class UserVacation extends Model
{
    ...
    public function registerVacation(int $userId, int $durationDays, float $costPaid): ?\stdClass
    {
        ...
        $endTimestamp = $this->parseTimestamp("+{$durationDays} days");
    ...
    private function parseTimestamp(string $expression): int
    {
        ...
        if ($timestamp === false) {
            throw new \RuntimeException('Unable to calculate vacation end date.');
        }
        return $timestamp;
    }
}
