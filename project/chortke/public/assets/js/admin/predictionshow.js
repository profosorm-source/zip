/* Prediction admin detail actions — CSP-safe, base-path-safe */
(function () {
  'use strict';
  let settleUrl = '';
  ...
  document.addEventListener('click', function (event) {
    ...
    if (actionBtn) {
      ...
      if (action === 'cancel') {
        if (!confirm('لغو بازی و برگشت کامل مبالغ انجام شود؟')) return;
        actionBtn.disabled = true;
        postJson(url, {}).then(data => {
          notify(data.message, !!data.success);
          if (data.success) setTimeout(() => location.href = location.href, 1200);
