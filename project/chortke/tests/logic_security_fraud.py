...
def test_security_L1_smoke_fraud_dashboard(client, assertions):
    ...
    assert_true(assertions, f"صفحه ضدتقلب ادمین HTTP {code}", code in (200, 302, 404))
...
def test_security_L2_valid_fingerprint_handshake(client, assertions):
    ...
    assert_true(assertions, f"هندشیک فینگرپرینت معتبر HTTP {code}", code in (200, 302, 404))
