...
class TestSuite:
    ...
    def summary(self):
        ...
        print(f"{'═' * 70}")
        print(f"  {GREEN}Passed: {passed}{RESET}  {RED}Failed: {failed}{RESET}  Total: {len(self.results)}\n")
        ...
        for tier_key, tier_name in TIERS.items():
            ...
            if total_tier == 0:
                ...
                passed_tier = sum(1 for r in tier_results if r.passed)
                status_str = f"{GREEN}✓ PASS{RESET}" if passed_tier == total_tier else f"{RED}✗ FAIL{RESET}"
                print(f"  {status_str} {tier_key} ({tier_name}): {passed_tier}/{total_tier}")
...
def ensure_test_user(email: str, role: str = 'user', verified: bool = True,
    ...
    if existing:
        ...
        db_insert(f"""
            ...
            VALUES ('{email.split('@')[0]}', '{email}', '{password_hash}', '{role}', 'active', {verified_at}, {'\'verified\'' if verified else '\'unverified\''}, NOW(), NOW())
        """)
