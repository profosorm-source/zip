...
def test_governance_L1_smoke_kpi_dashboard(client, assertions):
    ...
    assert_true(assertions, f"صفحه KPI ادمین HTTP {code}", code in (200, 302, 404))
...
def test_governance_L1_smoke_audit_trail_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه لاگ حسابرسی HTTP {code}", code in (200, 302, 404))
...
def test_governance_L1_smoke_system_logs_page(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body = client.get('/admin/system-logs')
    assert_true(assertions, f"صفحه لاگ‌های سیستم HTTP {code}", code in (200, 302, 404))
...
def test_governance_L1_smoke_bug_reports_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه گزارش باگ‌ها HTTP {code}", code in (200, 302, 404))
...
def test_governance_L1_smoke_levels_and_scores_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه مدیریت سطوح HTTP {code}", code in (200, 302, 404))
...
def test_governance_L2_admin_update_user_level(client, assertions):
    ...
    assert_true(assertions, f"ارتقای سطح کاربری ادمین HTTP {code}", code in (200, 302, 404))
...
def test_governance_L2_social_account_management(client, assertions):
    ...
    assert_true(assertions, f"داشبورد حساب‌های اجتماعی بارگذاری شد HTTP {code}", code in (200, 302, 404))
...
def test_governance_L4_sec_user_cannot_access_audit_logs(client, assertions):
    ...
    code2, body2 = client.get('/admin/system-logs')
...
def test_governance_L7_browser_kpi_and_audit_tables(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body = client.get('/admin/audit-trail')
    assert_true(assertions, f"جدول لاگ حسابرسی در مرورگر بارگذاری شد HTTP {code}", code in (200, 302, 404))
