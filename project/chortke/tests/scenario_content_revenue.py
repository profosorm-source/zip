...
def test_content_L1_smoke_user_content_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه مدیریت محتوای کاربر HTTP {code}", code in (200, 302, 404))
...
def test_content_L1_smoke_admin_content_page(client, assertions):
    ...
    assert_true(assertions, f"داشبورد محتوای ادمین HTTP {code}", code in (200, 302, 404))
...
def test_content_L1_smoke_admin_placements_page(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body = client.get('/admin/placements')
    assert_true(assertions, f"صفحه جایگاه‌های تبلیغاتی HTTP {code}", code in (200, 302, 404))
...
def test_content_L2_admin_create_placement_success(client, assertions):
    ...
    assert_true(assertions, f"ایجاد جایگاه تبلیغاتی ادمین HTTP {code}", code in (200, 302, 404))
...
def test_content_L7_browser_content_grid_interaction(client, assertions):
    ...
    code, body = client.get('/content')
    assert_true(assertions, f"جدول مطالب در مرورگر بارگذاری شد HTTP {code}", code in (200, 302, 404))
