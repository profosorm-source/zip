...
def test_sentry_L1_smoke_issue_detail(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body = client.get('/admin/sentry/issue/1')
    assert_true(assertions, f"صفحه جزئیات مشکل سنتری HTTP {code}", code in (200, 302, 404))
...
def test_sentry_L1_smoke_performance_monitoring(client, assertions):
    ...
    assert_true(assertions, f"صفحه پایش عملکرد HTTP {code}", code in (200, 302, 404))
...
def test_sentry_L2_acknowledge_issue(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body, _ = client.post(f'/admin/sentry/issue/{issue_id}/acknowledge', {'metadata': '{"note": "بررسی شد"}'})
    assert_true(assertions, f"تایید مشکل سنتری HTTP {code}", code in (200, 302, 404))
...
def test_sentry_L2_resolve_issue(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body, _ = client.post(f'/admin/sentry/issue/{issue_id}/resolve', {})
    assert_true(assertions, f"حل مشکل سنتری HTTP {code}", code in (200, 302, 404))
...
def test_sentry_L2_mute_issue(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body, _ = client.post(f'/admin/sentry/issue/{issue_id}/mute', {})
    assert_true(assertions, f"بی‌صدا کردن مشکل سنتری HTTP {code}", code in (200, 302, 404))
...
def test_sentry_L7_browser_sentry_performance_charts(client, assertions):
    ...
    client.login("admin@chortke.ir", DEFAULT_PASSWORD, admin=True)
    code, body = client.get('/admin/sentry/performance')
    assert_true(assertions, f"چارت‌های عملکرد در مرورگر بارگذاری شد HTTP {code}", code in (200, 302, 404))
