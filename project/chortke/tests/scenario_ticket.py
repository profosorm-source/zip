...
def test_ticket_L1_smoke_ticket_list_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه اصلی تیکت‌ها HTTP {code}", code in (200, 302, 404))
...
def test_ticket_L1_smoke_create_ticket_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه ایجاد تیکت HTTP {code}", code in (200, 302, 404))
...
def test_ticket_L1_smoke_direct_messages_page(client, assertions):
    ...
    assert_true(assertions, f"صفحه پیام‌های مستقیم HTTP {code}", code in (200, 302, 404))
...
def test_ticket_L7_browser_ticket_nav(client, assertions):
    ...
    code, body = client.get('/tickets')
    assert_true(assertions, f"جدول تیکت‌ها در مرورگر بارگذاری شد HTTP {code}", code in (200, 302, 404))
