...
class RetryPolicy
{
     ...
     * @throws Throwable
     */
    /** @param list<class-string>|null $retryOnExceptions */
    public function executeWithContext(string $context, callable $operation, ?array $retryOnExceptions = null): mixed
    {
        $attempt = 0;
    ...
    public static function acquireRetryBudget(string $context = 'global'): bool
    {
        ...
        $resilienceConfig = (array)config('resilience.retry_budget', []);
        $contexts = (array)($resilienceConfig['contexts'] ?? []);
        $global = (array)($resilienceConfig['global'] ?? []);
        $contextCfg = is_array($contexts[$context] ?? null) ? $contexts[$context] : [];

        $allowedPercentage = (int)($contextCfg['allowed_percentage'] 
                             ?? $global['allowed_percentage'] 
                             ?? 10);
                             
        $minAllowance = (int)($contextCfg['min_allowance'] 
                             ?? $global['min_allowance'] 
                             ?? 5);
        ...
        try {
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                $totalCalls += int_value($cache->get("retry_budget:{$context}:total_calls:{$bucket}", 0));
                $retries += int_value($cache->get("retry_budget:{$context}:retries:{$bucket}", 0));
            }
    ...
    {
        // Protocol/auth/validation failures are deterministic and must never
        // consume retry budget even though the domain exception hierarchy
        // ultimately extends RuntimeException.
        if ($exception instanceof \Core\Exceptions\PermanentFailure) {
            return false;
        }
