...
class Scheduler
{
    ...
    /** @var list<array{key: string, name: string, callback: callable, interval: int}> */
    private array $jobs = [];
    ...
    public function run(?string $onlyJobName = null): array
    {
        ...
        foreach ($this->jobs as $job) {
            ...
            $lockTtl = max(600, min(7200, (int)($job['interval'] ?? 600)));
            try {
    ...
    public function executeJobByName(string $name): mixed
    {
        foreach ($this->jobs as $job) {
            if ($job['name'] === $name) {
                ...
                // Finding #14 Fix: Dynamic lock TTL based on job interval and type
                $lockTtl = max(600, min(7200, (int)($job['interval'] ?? 600)));
