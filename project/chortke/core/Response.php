...
class Response
{
    ...
    {
        $this->content = str_value($content);
        return $this;
    }
    ...
    {
        // Preserve a status explicitly set through status()/setStatusCode().
        // A fresh Response already defaults to 200, while chained responses
        // such as status(405)->json(...) must not be reset back to 200.
        if ($statusCode !== null) {
    ...
    private function validateRedirectUrl(string $url): bool
    {
        ...
        $baseHost = (string)parse_url(app(UrlGenerator::class)->origin(), PHP_URL_HOST);
    ...
    private function validateFilePath(string $filePath): bool
    {
        ...
        // ✅ Ensure file is within allowed base directories with trailing separator (Issue #25 Fix)
        $paths = app(PathResolver::class);
        $allowedBases = [
            realpath($paths->public('uploads')),
            realpath($paths->storage()),
        ];
        ...
        foreach ($allowedBases as $base) {
            if ($base !== false) {
                ...
                if (str_starts_with($realPath, $baseDir) || $realPath === rtrim($base, '/\\')) {
                    $isAllowed = true;
                    break;
                }
            }
    ...
    public function back(): void
    {
        ...
        $referer = $_SERVER['HTTP_REFERER'] ?? $generator->to('/');
        $scheme = parse_url($referer, PHP_URL_SCHEME);
        $host = parse_url($referer, PHP_URL_HOST);
        $port = parse_url($referer, PHP_URL_PORT);
        if (is_string($scheme) && is_string($host)) {
            $refererOrigin = strtolower($scheme . '://' . $host . (is_int($port) ? ':' . $port : ''));
            if (!hash_equals(strtolower($generator->origin()), $refererOrigin)) {
                $referer = $generator->to('/');
            }
        } elseif (!is_string($referer) || !str_starts_with($referer, '/') || str_starts_with($referer, '//')) {
            $referer = $generator->to('/');
        }
    ...
    private function sendHeaders(): void
    {
        ...
        // Permanent Architecture: Single source of truth. If SecurityHeadersMiddleware or
        // a controller already set Content-Security-Policy in $this->headers, do not emit a second CSP header.
        if (!isset($this->headers['Content-Security-Policy']) && !isset($this->headers['content-security-policy'])) {
            try {
                ...
                $assetOrigin = app(UrlGenerator::class)->assetOrigin();
                if ($assetOrigin !== app(UrlGenerator::class)->origin()) {
                    $cdnWhitelist[] = $assetOrigin;
                }
