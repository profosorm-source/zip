    ...
    public function instance(string $abstract, object $object): void
    {
        if ((class_exists($abstract) || interface_exists($abstract)) && !($object instanceof $abstract)) {
            throw new \InvalidArgumentException(
                "[Container] Instance for '{$abstract}' has incompatible type " . get_class($object) . '.'
            );
        }
    ...
    public function make(string $abstract): object
    {
        ...
        try {
            ...
            if (isset($this->scopedBindings[$abstract])) {
                ...
                $result = $this->scopedInstances[$abstract];
            } elseif (array_key_exists($abstract, $this->singletons)) {
                // Singleton cache
                if ($this->singletons[$abstract] === null) {
                    $instance = $this->resolve($abstract);
                    $this->singletons[$abstract] = $this->applyExtenders($abstract, $instance);
                }
                $result = $this->singletons[$abstract];
            } else {
                $result = $this->applyExtenders($abstract, $this->resolve($abstract));
            }
            /** @var T $result */
            return $result;
        } finally {
    ...
    private function resolve(string $abstract): object
{
    ...
    // pre-built object binding (Closure was handled above)
    if (is_object($concrete)) {
        if ((class_exists($abstract) || interface_exists($abstract)) && !($concrete instanceof $abstract)) {
            throw new \RuntimeException(
                "[Container] Object binding for '{$abstract}' has incompatible type " . get_class($concrete) . '.'
            );
        }
        return $concrete;
    }
        ...
        foreach ($parameters as $param) {
            ...
            if ($type instanceof \ReflectionNamedType && !$type->isBuiltin()) {
                try {
                    /** @var class-string $typeName */
                    $typeName = $type->getName();
                    $resolvedArgs[] = $this->make($typeName);
                    continue;
                } catch (\RuntimeException $e) {
