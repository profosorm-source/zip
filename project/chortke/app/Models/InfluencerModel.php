...
class InfluencerModel extends Model
{
    ...
    public function getVerifiedProfiles(array $filters = [], string $sort = 'priority', int $limit = 20, int $offset = 0): array
    {
        ...
        if (!empty($filters['min_followers'])) {
            $where[] = "ip.follower_count >= ?"; $params[] = (int)(is_numeric($filters['min_followers'] ?? null) ? $filters['min_followers'] : 0);
        }
        if (!empty($filters['max_price'])) {
            $where[] = "ip.story_price_24h <= ?"; $params[] = (float)(is_numeric($filters['max_price'] ?? null) ? $filters['max_price'] : 0);
        }
    ...
    public function countVerifiedProfiles(array $filters = []): int
    {
        ...
        if (!empty($filters['min_followers'])) { $where[] = "ip.follower_count >= ?"; $params[] = (int)(is_numeric($filters['min_followers'] ?? null) ? $filters['min_followers'] : 0); }
        if (!empty($filters['max_price'])) { $where[] = "ip.story_price_24h <= ?"; $params[] = (float)(is_numeric($filters['max_price'] ?? null) ? $filters['max_price'] : 0); }
        if (!empty($filters['search'])) {
            $where[] = "(ip.username LIKE ? OR ip.bio LIKE ? OR u.full_name LIKE ?)";
            $s = '%' . $filters['search'] . '%'; $params[] = $s; $params[] = $s; $params[] = $s;
        }
