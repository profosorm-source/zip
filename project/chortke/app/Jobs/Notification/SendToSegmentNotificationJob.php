...
public function handle(
        ...
        while (true) {
            $userIds = $this->db->fetchAll(
                $segmentQuery . " ORDER BY id ASC LIMIT ? OFFSET ?",
                array_merge(array_values($filters), [self::BATCH_SIZE, $offset])
            );
    ...
    /** @param array<string, mixed> $filters */
    private function buildSegmentQuery(string $segment, array $filters): string
    {
        ...
        switch ($segment) {
            ...
            default:
                // Custom segment — pass through any additional filters
                $extra = '';
