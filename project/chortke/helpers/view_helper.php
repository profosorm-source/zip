...
if (!function_exists('include_view_content')) {
    /** @param array<string, mixed> $data */
    function include_view_content(string $contentView = '', array $data = []): void
    {
        if ($contentView === '') return;
        extract($data);
        $file = view_path($contentView);
        if (file_exists($file)) {
            include $file;
        }
    }
...
if (!function_exists('view')) {
    ...
    function view(string $viewName, array $data = []): string
    {
        ...
        $globals['errors']       = (array)($session->getFlash('errors') ?? []);
        $globals['old']          = (array)($session->getFlash('old')    ?? []);
        ...
        // View data has a strict type-preservation contract: the rendering
        // boundary must never rewrite a controller/service DTO. Objects remain
        // objects, arrays remain arrays, and nested values keep their identity.
        // A legacy view that needs an array must receive one explicitly from its
        // controller/presenter instead of relying on global implicit coercion.
        $viewVars = array_merge($globals, $data);
        ...
        $html = ob_get_clean();
        if (!is_string($html)) {
            throw new \RuntimeException("Unable to capture rendered view: {$viewName}");
        }
        echo $html;
...
if (!function_exists('e')) {
    ...
    {
        return htmlspecialchars(str_value($value), ENT_QUOTES, 'UTF-8');
    }
}
...
if (!function_exists('user_sidebar_badges')) {
     ...
     *
     * @return array{disputes_open:int,influencer_orders_pending:int,unread_notifications?:int,top_notifications?:list<\stdClass>}
     */
