...
try {
    ...
    if ($redis instanceof \Core\Redis && $redis->isAvailable()) {
        try {
            ...
            if ($client === null) throw new \RuntimeException('Redis client unavailable for cron lock.');
            $lockAcquired = $client->set($distributedLockKey, $distributedLockToken, ['nx', 'ex' => 60]);
            if (!$lockAcquired) {
    ...
    register_shutdown_function(function () use ($redis, $distributedLockKey, $distributedLockToken, $lockHandle, $lockFile) {
        if ($redis instanceof \Core\Redis && $redis->isAvailable()) {
            try {
                $client = $redis->getClient();
                if ($client !== null && $client->get($distributedLockKey) === $distributedLockToken) {
                    $client->del($distributedLockKey);
                }
            } catch (\Throwable $e) {}
        }
        ...
        if (isset($lockHandle) && $lockHandle !== null) {
            ...
            fclose($lockHandle);
            if (is_string($lockFile)) @unlink($lockFile);
        }
    });
    ...
    foreach ($results as $name => $resultValue) {
        if (!is_array($resultValue)) throw new \UnexpectedValueException('Scheduler dispatch result must be an array.');
        $result = $resultValue;
        $status = str_value($result['status'] ?? 'unknown');
        $icon   = match($status) {
            'queued'  => '✓',
            'error'   => '✗',
            'skipped' => '⟳',
        ...
        if ($status === 'error') {
            echo ' - ' . str_value($result['message'] ?? '');
        } elseif ($status === 'skipped') {
            echo ' - ' . str_value($result['reason'] ?? '');
        }
        echo PHP_EOL;
    }
