...
$r->group(['prefix' => '/api/v1', 'middleware' => [ApiRequestMiddleware::class, \App\Middleware\RateLimitMiddleware::class]], function ($r) {
    ...
    $r->group(['middleware' => [ApiAuthMiddleware::class . ':user.write']], function ($r) {
        ...
        $r->post('/user/2fa/setup', [ApiUserController::class, 'twoFactorSetup']);
    ...
    $r->post('/security/event', [\App\Controllers\Api\SecurityController::class, 'logEvent'], [\App\Middleware\RateLimitMiddleware::class]);
    $r->get('/user/referral/stats', [\App\Controllers\User\ReferralController::class, 'stats']);
