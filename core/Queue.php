...
class Queue
{
    ...
    private function initRedis(): void
    {
        try {
            ...
            $this->useRedis = false;
            if (function_exists('logger')) { logger()->warning('queue.redis_init_failed', ['error' => $e->getMessage()]); }
        }
    }
    ...
    public function push(string $job, array $data = [], ?string $queue = null, int $delay = 0): bool
    {
        ...
        if ($normalizedJob === 'App\\Jobs\\UpdateFraudScoreJob') {
            ...
            $domain = strval($data['domain'] ?? 'fraud');
        ...
        if ($this->useRedis) {
            try {
                ...
                if (function_exists('logger')) { logger()->error('queue.redis.push_failed_falling_back', [
                        'job' => $job,
                        'queue' => $queue,
                        'error' => $e->getMessage(),
                    ]); }
    ...
    protected function runRedisEval(string $script, array $args, int $numKeys): mixed
    {
        ...
        if ($this->redis instanceof \Mockery\MockInterface || \method_exists($this->redis, 'evaluate')) {
            return $this->redis->{'evaluate'}($script, $args, $numKeys); // @phpstan-ignore-line
        }
...
LUA;
                ...
                $decodedJobData = json_decode($jobDataJson, true);
                ...
                if (function_exists('logger')) { logger()->error('queue.redis.pop_failed_falling_back', [
                        'queue' => $queue,
                        'error' => $e->getMessage(),
                    ]); }
    ...
    public function delete(int $id): bool
    {
        if ($this->useRedis) {
            try {
                ...
                if ($jobDataJson) {
                    $jobData = (array)(json_decode($jobDataJson, true) ?? []);
        ...
        if ($this->useRedis) {
            try {
                ...
                if ($jobDataJson) {
                    $jobData = (array)(json_decode($jobDataJson, true) ?? []);
                    $queue = $jobData['queue'] ?? 'default';
                    $attempts = intval($jobData['attempts'] ?? 1);
        ...
        try {
            ...
            if ($this->useRedis) {
                try {
                    ...
                    if (function_exists('logger')) { logger()->error('queue.redis.fail_read_failed', ['id' => $id, 'error' => $e->getMessage()]); }
                }
            }
            ...
            if (function_exists('logger')) { logger()->critical('queue_job_failed_dlq_moved', [
                    'job_id' => $id,
                    'queue' => $queueName,
                    'payload' => $payloadStr,
