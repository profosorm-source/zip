...
class Request
{
    ...
    public function __construct() {
        ...
        $maxBody = (int) config('request.max_body_bytes', 12 * 1024 * 1024); // 12 MB default; feature-level validators enforce stricter limits
...
private function parseBody(): array
{
    ...
    public function isSecure(): bool
    {
        ...
        foreach ($trustedProxies as $proxy) {
            if (ip_in_range($clientIp, (string)$proxy)) {
                $isTrusted = true;
                break;
            }
        }
    ...
    private function parseUri(): string
    {
        ...
        if ($basePath !== '' && $basePath !== '/' && str_starts_with($uri, (string)$basePath)) {
            $uri = substr($uri, strlen((string)$basePath));
        }
