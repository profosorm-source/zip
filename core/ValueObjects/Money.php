...
readonly class Money
{
    ...
    public function __construct(string|int $amount, string $currency = self::DEFAULT_CURRENCY) {
        // 🛡️ اجرای سفت‌وسخت قانون عدم استفاده از float در بخش‌های مالی
        if (is_float($amount) || is_double($amount)) {
            throw new InvalidArgumentException("استفاده از نوع داده float در محاسبات مالی و پول اکیداً ممنوع است. از رشته یا عدد صحیح استفاده کنید.");
        }
    ...
    {
        if (is_float($multiplier) || is_double($multiplier)) {
            throw new InvalidArgumentException("استفاده از نوع داده float در محاسبات مالی و پول اکیداً ممنوع است.");
        }
        $scale = $this->getScale();
    ...
    {
        if (is_float($divisor) || is_double($divisor)) {
            throw new InvalidArgumentException("استفاده از نوع داده float در محاسبات مالی و پول اکیداً ممنوع است.");
        }
        $scale = $this->getScale();
        $dStr  = (string)$divisor;
