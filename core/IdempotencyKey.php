...
class IdempotencyKey
{
    ...
    public function check(string $key, int $userId, string $action, ?array $requestData = null, int $retryCount = 0): array
    {
        ...
        try {
            ...
            if (!$wasInserted) {
                $storedSignature = (array)(json_decode($existing['request_data'] ?? '', true) ?? []);
     ...
     * @throws \Exception
     */
    public function wrap(string $key, int $userId, string $action, callable $callback, ?array $requestData = null)
    {
